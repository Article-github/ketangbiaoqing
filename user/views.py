...
from .models import User, Black_list
...
def get_blackusers(request):
    ...
    data = []
    if keyword is None:
        results_obj = Black_list.objects.all()
    else:
        results_obj = Black_list.objects.filter(name__contains=keyword).all()
...
def black(request):
    ...
    role = int(request.session['role'])
    user_id= request.session['user_id']
    return render(request, 'black_list.html', locals())
